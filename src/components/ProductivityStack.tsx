...
const ProductivityStack = () => {
  ...
  const SkeletonLine = ({ width = "100%", className = "", hoverable = false }: { width?: string; className?: string; hoverable?: boolean }) => (
    <div 
  ...
  const ToolIcon = ({ icon: Icon, label, color = "muted" }: { icon: any; label: string; color?: string }) => {
    ...
    return (
      <div className="flex items-center gap-2.5">
        <div className={`w-7 h-7 rounded-lg flex items-center justify-center ${colorClasses[color]} transition-transform duration-200 group-hover:scale-105`}>
  ...
  const StatusIndicator = ({ status, label }: { status: "active" | "idle" | "running"; label: string }) => {
    const statusConfig = {
      active: { color: "bg-emerald-400", text: "text-emerald-300", pulse: true },
    ...
    return (
      <div className="flex items-center gap-2 px-2 py-1 rounded-full bg-muted/20 transition-colors duration-200 hover:bg-muted/30">
        <div className="relative">
          ...
          {config.pulse && (
            <div className={`absolute inset-0 w-2 h-2 rounded-full ${config.color} animate-ping opacity-60`} />
          )}
        </div>
        <span className={`text-[11px] font-semibold tracking-wide uppercase ${config.text}`}>{label}</span>
      </div>
    );
  };
        ...
        {[
          ...
          <div key={col.name} className="flex-1 space-y-2">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] text-muted-foreground/70 uppercase tracking-wider font-medium">{col.name}</span>
              <span className="text-[10px] text-muted-foreground/50 font-mono">{col.count}</span>
            </div>
            {[...Array(col.count)].map((_, j) => (
              <motion.div 
                ...
                animate={col.color === "blue" ? { opacity: [0.85, 1, 0.85] } : {}}
                transition={{ duration: 2, repeat: Infinity }}
              />
            ))}
          </div>
      ...
      <div className="relative mt-2">
        ...
        <svg className="w-20 h-20 -rotate-90" viewBox="0 0 100 100">
          ...
          <motion.circle
            ...
            strokeLinecap="round"
            animate={{ strokeDashoffset: [264 * 0.25, 264 * 0.20, 264 * 0.25] }}
            transition={{ duration: 10, repeat: Infinity }}
          />
        </svg>
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-lg font-mono font-semibold text-foreground/90">45:00</span>
        </div>
      </div>
          ...
          <div key={step.label} className="space-y-1.5 group/step">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                <step.icon className={`w-3.5 h-3.5 transition-colors duration-200 ${step.progress === 100 ? 'text-amber-400' : 'text-muted-foreground/50 group-hover/step:text-muted-foreground/70'}`} />
                <span className="text-xs text-muted-foreground/80 font-medium">{step.label}</span>
              </div>
              <span className="text-xs text-muted-foreground/60 font-mono">{step.progress}%</span>
            </div>
  ...
  const DevToolsContent = () => (
    <div className="space-y-3 h-full">
      <div className="flex items-center gap-3">
        <ToolIcon icon={Code2} label="VS Code" color="cyan" />
        <span className="text-muted-foreground/40">•</span>
      ...
      <div className="rounded-xl bg-muted/20 border border-border/40 p-3 space-y-2 mt-3">
        ...
        <div className="flex gap-1.5 mb-3">
          <div className="px-2.5 py-1 rounded-md bg-cyan-500/25 text-[10px] text-cyan-300 font-semibold">App.tsx</div>
          <div className="px-2.5 py-1 rounded-md bg-muted/20 text-[10px] text-muted-foreground/50">utils.ts</div>
        </div>
        ...
        <div className="space-y-2 font-mono">
          {[
            ...
            <div key={i} className="flex items-center gap-3 group/line">
              <span className="text-[10px] text-muted-foreground/40 w-4 shrink-0 font-mono">{i + 1}</span>
              <div style={{ marginLeft: line.indent }}>
                <SkeletonLine width={line.width} className={`h-2 ${line.highlight ? "bg-cyan-500/30" : ""}`} hoverable />
              </div>
            </div>
          ))}
        </div>
        ...
        {[
          ...
          >
            ...
            <div className="flex-1 space-y-1.5">
              <SkeletonLine width={msg.width} className={`h-2 ${msg.unread ? "bg-rose-300/40" : ""}`} hoverable />
              <SkeletonLine width="40%" className="h-1.5" />
            </div>
            {msg.unread && <div className="w-2.5 h-2.5 rounded-full bg-rose-400" />}
          </motion.div>
        ))}
      </div>
      ...
      <div className="space-y-3 mt-3">
        ...
        <div className="flex items-center justify-between gap-2">
          {[
            ...
            { label: "In Progress", count: 2, color: "bg-blue-500/50" },
            { label: "Done", count: 7, color: "bg-emerald-500/50" }
          ].map((status) => (
            <div key={status.label} className="flex-1 flex flex-col items-center group/status">
              <div className={`w-full h-10 rounded-lg ${status.color} flex items-center justify-center transition-all duration-200 group-hover/status:scale-[1.03]`}>
                <span className="text-sm font-bold text-foreground/80">{status.count}</span>
              </div>
              <span className="text-[10px] text-muted-foreground/60 mt-1.5 font-medium">{status.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex items-center gap-2.5 p-2 rounded-lg bg-muted/20 border border-border/30 transition-colors duration-200 hover:bg-muted/30">
  ...
  const getGlowStyles = (glowColor: string, isHovered: boolean) => {
    ...
    const glowColors: Record<string, string> = {
      violet: "0 0 40px rgba(139, 92, 246, 0.15), 0 0 80px rgba(139, 92, 246, 0.05)",
      blue: "0 0 40px rgba(59, 130, 246, 0.15), 0 0 80px rgba(59, 130, 246, 0.05)",
      emerald: "0 0 40px rgba(52, 211, 153, 0.15), 0 0 80px rgba(52, 211, 153, 0.05)",
      amber: "0 0 40px rgba(251, 191, 36, 0.15), 0 0 80px rgba(251, 191, 36, 0.05)",
          ...
          {panels.map((panel, index) => (
            ...
            >
              <div
                ...
                `}
                style={getGlowStyles(panel.glowColor, hoveredPanel === panel.id)}
              >
                {/* Panel Header */}
                <div className="mb-3">
